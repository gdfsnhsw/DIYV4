...
repo=$repo6                                        #默认调用 shufflewzc_faker2 仓库脚本日志
...
BreakHelpNum="35-102"  ## 屏蔽账号序号或序号区间
...
export_codes_sub() {
    ...
    [ $task_name = "$repo"_jd_fruit ] && strictnum=7
    [ $task_name = "$repo"_jd_pet ] && strictnum=7
    [ $task_name = "$repo"_jd_plantBean ] && strictnum=7
    [ $task_name = "$repo"_jd_dreamFactory ] && strictnum=20
    [ $task_name = "$repo"_jd_jdfactory ] && strictnum=20
        ...
        if [[ ${#code[*]} -gt 0 ]]; then
            ...
            *) ## 按编号优先
                ...
                for ((m = 0; m < ${#pt_pin[*]}; m++)); do
                    tmp_for_other=""
                    j=$((m + 1))
                    for ((n = 0; n < $user_sum && n < strictnum; n++)); do
                        [[ $m -eq $n ]] && continue
                        ((addnum++))
                        k=$((addnum))
                        [[ $addnum -eq $ck_strictnum ]] && addnum=0 
                        if [[ $BreakHelpType = "1" ]]; then
                            if [ "$BreakHelpNumVerify" = "" ]; then
                                for ((t = 0; t < ${#BreakHelpNumArray[*]}; t++)); do
                                    [[ "${BreakHelpNumArray[t]}" = "$k" ]] && continue 2
...
export_all_codes() {
    ...
    if [ "$ps_num" -gt $proc_num ]; then
        ...
        for ((i = 0; i < ${#name_js[*]}; i++)); do
            echo -e "\n## ${name_chinese[i]}："
            export_codes_sub "${name_js[i]}" "${name_config[i]}" "${name_chinese[i]}" $strictnumset $ck_strictnumset
        done
    fi
}
